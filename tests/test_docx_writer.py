"""Tests for themis/tools/docx_writer.py"""
import os
import pytest
...
def test_write_docx_no_themis_footer_in_filing(tmp_path):
    # The filing .docx must NOT contain internal Themis metadata.
    # Matter ID, "Generated by Themis", and "Phase 5 draft" are lawyer notes
    # and belong in <stem>_notes.docx, not in the court-filed document.
    out = str(tmp_path / "filing.docx")
    write_docx(_state(), out)
    doc = Document(out)
    full_text = "\n".join(p.text for p in doc.paragraphs)
    assert "M-DOC-001" not in full_text
    assert "Generated by Themis" not in full_text
    assert "Phase 5 draft" not in full_text


def test_write_docx_court_header_present(tmp_path):
    # The filing .docx must start with a formal court header derived from jurisdiction.
    out = str(tmp_path / "header.docx")
    write_docx(_state(), out)
    doc = Document(out)
    full_text = "\n".join(p.text for p in doc.paragraphs)
    # jurisdiction="Delhi High Court" → "IN THE HIGH COURT OF DELHI AT NEW DELHI"
    assert "IN THE HIGH COURT" in full_text.upper()
