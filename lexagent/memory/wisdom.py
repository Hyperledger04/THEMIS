...
# to ~/.lexagent/wisdom.md. Each matter contributes a few lines — over time the
...
def wisdom_path(home_dir: str = "~/.lexagent") -> Path:
    return Path(home_dir).expanduser() / _WISDOM_FILENAME


def load_wisdom(home_dir: str = "~/.lexagent") -> str:
...
def _append_wisdom(new_yaml: str, home_dir: str = "~/.lexagent") -> None:
    p = wisdom_path(home_dir)
