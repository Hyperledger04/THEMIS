# Telegram gateway for LexAgent — Phase 8 UX overhaul.
#
...
class TelegramSession:
    """Per-user session state mapping Telegram user_id → LexAgent matter context."""
    matter_id: str
...
# Pattern matching internal LexAgent matter IDs (e.g. M-FC4A838E)
...
def _get_or_create_session(user_id: int, cfg: LexConfig) -> TelegramSession:
    ...
    if not cfg.postgres_url:
        try:
            from lexagent.memory.session_store import list_sessions, get_session_state
...
async def _run_graph_for_user(
    ...
    progress_msg = await message.reply_text("⚖️ LexAgent is working…")
...
def _persist_session(session: TelegramSession, cfg: LexConfig) -> None:
    ...
    if not session.graph_state or cfg.postgres_url:
        ...
        return
    try:
        from lexagent.memory.session_store import update_session
...
async def cmd_matters(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        from lexagent.memory.session_store import search_sessions, list_sessions
...
async def cmd_setup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    from lexagent.gateway.setup_wizard import start_wizard
    session = _get_or_create_session(user.id, cfg)
...
async def cmd_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        from lexagent.scheduler.reminders import add_reminder_and_schedule
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("setup:"):
        from lexagent.gateway.setup_wizard import handle_wizard_callback
...
async def _handle_post_draft_action(
    ...
    if action == "drive":
        ...
        try:
            from lexagent.gateway.integrations import upload_to_drive
    ...
    if action == "ecourts":
        ...
        try:
            from lexagent.gateway.integrations import lookup_ecourts
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if session.in_setup:
        from lexagent.gateway.setup_wizard import handle_wizard_text
...
async def _handle_post_draft_text_input(
    ...
    if action == "email_recipient":
        ...
        try:
            from lexagent.gateway.integrations import send_draft_email
        ...
        try:
            from lexagent.gateway.integrations import forward_draft
        ...
        try:
            from lexagent.gateway.integrations import send_docusign
...
def run_telegram_bot(cfg: Optional[LexConfig] = None) -> None:
    ...
    console.print("[bold cyan]⚖ LexAgent Telegram Gateway — Phase 8[/bold cyan]")
