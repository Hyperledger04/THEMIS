...
def _default_to_chat(ctx: typer.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        ...
        from lexagent.chat import run_chat
...
@app.command()
def chat() -> None:
    """Chat with LexAgent in natural language — draft, research, manage matters."""
    cfg = LexConfig()
    from lexagent.nodes._llm import setup_litellm_cache
    setup_litellm_cache(cfg)
    from lexagent.chat import run_chat
...
def draft(
    ...
    from lexagent.agents.registry import parse_agent_mention, load_agent
    ...
    asyncio.run(_run_draft(brief, session_matter_id, cfg, prior_state, output_path, agent_config))
...
async def _run_draft(
    ...
    if prior_state:
        ...
        state = _blank_state(initial_brief, matter_id, output_path, agent_config)
...
config_app = typer.Typer(name="config", help="Configure LexAgent — provider, model, keys.")
...
def config(ctx: typer.Context) -> None:
    ...
    from lexagent.providers import list_providers

    cfg = LexConfig()
    saved = _load_config_yaml(cfg.config_yaml)

    console.print()
    console.print(Rule(title="[bold cyan] LexAgent Provider Setup [/bold cyan]", style="cyan"))
    console.print()
...
def config_show() -> None:
    """Print current LexAgent configuration."""
    ...
    console.print()
    console.print(Panel(table, title="[bold cyan]LexAgent Config[/bold cyan]", border_style="cyan"))
...
def agent_create() -> None:
    ...
    Saved to ~/.lexagent/agents/ and immediately usable with @handle.
    """
    from lexagent.agents.registry import create_agent, load_agent
    from lexagent.agents.faces import FACES, list_faces_table
    ...
    console.print(
        Panel(
            "Create a custom advocate persona that LexAgent will embody.\n"
...
def agent_delete(
    ...
    from lexagent.agents.registry import delete_agent
...
def research(
    ...
    console.print(Panel(
        ...
        title="[bold cyan]⚖ LexAgent Research[/bold cyan]",
        border_style="cyan",
    ))
    console.print()
...
def query(
    ...
    from lexagent.tools.kb_query import search_kb, search_kb_qdrant
...
def qa(
    ...
    cfg = LexConfig()
    from lexagent.nodes.document_qa import run_document_qa_session
...
def contract_review_cmd(
    ...
    playbook_prompt = ""
    if playbook:
        from lexagent.contract.playbook import load_playbook, playbook_to_prompt
...
def contract_draft_cmd(
    ...
    if playbook:
        from lexagent.contract.playbook import load_playbook, playbook_to_prompt
        playbooks_dir = "~/.lexagent/playbooks"
...
_PLAYBOOKS_DIR = "~/.lexagent/playbooks"


@playbook_app.command("list")
def playbook_list() -> None:
    """List all contract playbooks (bundled + custom)."""
    from lexagent.contract.playbook import list_playbooks
...
def playbook_show(
    ...
    from lexagent.contract.playbook import load_playbook
...
def playbook_add() -> None:
    ...
    from lexagent.contract.playbook import create_playbook
...
def playbook_delete(
    ...
    from lexagent.contract.playbook import delete_playbook
...
def wisdom() -> None:
    ...
    relevant statutes, court-specific notes) to ~/.lexagent/wisdom.md.
    """
    cfg = LexConfig()
    from lexagent.memory.wisdom import wisdom_path, load_wisdom
...
def help_cmd() -> None:
    """
    Show a full reference card of all LexAgent commands and usage examples.
    """
    from rich.columns import Columns

    console.print()
    console.print(Panel(
        "[bold cyan]⚖ LexAgent[/bold cyan] — AI agent for Indian litigation\n"
    ...
    console.print(Panel(
        "[dim]Docs:[/dim] [bold]https://github.com/your-org/lexagent[/bold]\n"
