"""
LexAgent natural language chat interface.
...
_SYSTEM_PROMPT = """You are LexAgent, an AI assistant built for Indian lawyers.
...
async def run_chat(cfg: LexConfig) -> None:
    ...
    console.print(
        Panel(
            f"[bold cyan]⚖ LexAgent[/bold cyan]  Natural Language Mode{resumed}\n"
    ...
    while True:
        ...
        from lexagent.agent.loop import LexAgentLoop
        loop = LexAgentLoop(
            model=chat_model,
            tools=_TOOLS,
...
async def _tool_draft(
    ...
    from lexagent.ui.live import LiveStatus
...
async def _tool_save_document(matter_id: str, filename: str, cfg: LexConfig) -> str:
    ...
    if not safe_name:
        safe_name = f"LexAgent_Draft_{matter_id}.docx"
...
def _tool_search_kb(query: str, cfg: LexConfig) -> str:
    from lexagent.tools.kb_query import search_kb
