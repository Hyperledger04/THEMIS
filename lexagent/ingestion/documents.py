...
MVP storage: local filesystem under ~/.lexagent/matters/{matter_id}/documents/.
...
_STORAGE_ROOT = Path(os.environ.get("LEXAGENT_STORAGE_ROOT", Path.home() / ".lexagent" / "documents"))
...
def _try_ocr_page(file_path: Path, page_number: int) -> str:
    ...
    """
    try:
        from lexagent.ingestion.ocr import ocr_pdf_page
