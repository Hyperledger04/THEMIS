"""
LexAgentLoop — the conversational tool-call loop for the chat interface.
...
WHY a separate class from LangGraph's StateGraph:
  ...
  - LexAgentLoop = the conversational shell in chat.py. Handles "which tool should I
...
from lexagent.agent.budget import IterationBudget
...
class LexAgentLoop:
    ...
    async def _one_llm_turn(self, messages: list[dict]) -> list[dict]:
        ...
        """
        console.print()
        console.print("[bold green]LexAgent[/bold green]  ", end="")
        sys.stdout.flush()
