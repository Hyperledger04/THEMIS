...
_FIELD_TO_STATE: dict[str, str] = {
    ...
    # S.138 NI Act fields
    "accused_entity_type": "accused_entity_type",
...
def _resolve_matter_key(matter_type: str) -> Optional[str]:
    ...
    if "138" in mt or "s138" in mt or "s.138" in mt or "cheque" in mt or "ni act" in mt or "dishonour" in mt or "negotiable" in mt:
        return "s138_complaint"
...
def _build_system_prompt(matter_type: Optional[str], bank_section: str) -> str:
    return f"""You are Themis's intake specialist for Indian lawyers.
...
async def run(state: LexState) -> dict:
    ...
    try:
        ...
        # Build exhibit registry the moment intake completes.
        # WHY: Must be done here (not in draft.py) so the registry is available
        # to BOTH the main complaint draft AND the affidavit sub-document call,
        # ensuring exhibit labels are identical across all filing documents.
        if updates.get("intake_complete") and not state.get("exhibit_registry"):
            matter_facts = {
                "second_dishonour": merged.get("key_clauses") and
                    any("second" in str(c).lower() for c in (merged.get("key_clauses") or [])),
                "part_payments": merged.get("key_clauses") and
                    any("part payment" in str(c).lower() or "partial" in str(c).lower()
                        for c in (merged.get("key_clauses") or [])),
            }
            registry = build_exhibit_registry(merged.get("matter_type"), matter_facts)
            if registry:
                updates["exhibit_registry"] = registry
...
# ---------------------------------------------------------------------------
# Exhibit registry builder
# ---------------------------------------------------------------------------

# Standard exhibit sequence for S.138 NI Act complaints.
# WHY: Labels are built here — before any LLM call — so that every downstream
# document (complaint body, affidavit, list of documents) uses the identical
# EX-CW1/A format. The LLM is told to use these labels verbatim; it must not
# invent its own (e.g. "Annexure A").
_S138_EXHIBIT_SEQUENCE = [
    ("invoice",         "Invoice / agreement giving rise to the legally enforceable debt"),
    ("cheque",          "Dishonoured cheque"),
    ("return_memo_1",   "Bank return memo — first dishonour"),
    ("demand_notice",   "Legal demand notice"),
    ("postal_receipt",  "Postal receipt / speed post tracking / proof of service"),
    ("return_memo_2",   "Bank return memo — second dishonour"),
    ("payment_receipt", "Part payment receipt(s)"),
    ("correspondence",  "Additional correspondence (WhatsApp, email, etc.)"),
]

_ALPHA = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"


def build_exhibit_registry(matter_type: Optional[str], matter_facts: dict) -> dict:
    """
    Build a canonical exhibit registry for the matter before drafting begins.

    Returns a dict:
      {"invoice": "EX-CW1/A", "cheque": "EX-CW1/B", ...}

    For S.138 complaints the sequence is fixed (per court convention).
    For other matter types a generic Annexure P-N sequence is returned.

    matter_facts is a subset of LexState fields (used to skip exhibits where the
    underlying document is confirmed absent — e.g. no second dishonour).
    """
    if not matter_type:
        return {}
    ...
    if "138" in mt or "cheque" in mt or "dishonour" in mt or "ni act" in mt:
        registry: dict[str, str] = {}
        alpha_idx = 0
        for key, _description in _S138_EXHIBIT_SEQUENCE:
            # Skip second return memo if there was no second dishonour
            if key == "return_memo_2" and not matter_facts.get("second_dishonour"):
                continue
            # Skip payment receipt if no part payments mentioned
            if key == "payment_receipt" and not matter_facts.get("part_payments"):
                continue
            label = f"EX-CW1/{_ALPHA[alpha_idx]}"
            registry[key] = label
            alpha_idx += 1
        return registry

    # Generic fallback: Annexure P-1, P-2, ... for other matter types
    # This lets any future skill inject its own exhibit list as a numbered sequence.
    return {}
