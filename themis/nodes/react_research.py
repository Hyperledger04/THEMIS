...
4. Judgment auto-cache: verified findings written to
   ~/.themis/judgments/{docid}.txt so the precedent board (Phase R9) can
...
async def _fetch_with_fallback(
    ...
    Strategy:
    1. Check local judgment cache (~/.themis/judgments/{tid}.txt).
    ...
    if docid and cfg.kanoon_api_key:
        try:
            from themis.tools.kanoon_api import KanoonAPIClient
    ...
    if not full_text and url and cfg.kanoon_fallback_playwright:
        ...
        try:
            from themis.tools.kanoon_fallback import fetch_doc_playwright
...
async def _run_tavily_search(query: str, cfg: LexConfig) -> list[dict]:
    ...
    try:
        from themis.tools.tavily_search import web_search_async
